...
// Date de modification : 2019-09-10
            ...
            if (input != "none")
            {
                ...
                else if (input.StartsWith("info"))
                {
                    ...
                    if (argus[0] == "info")
                    {
                        ...
                        {
                            BankAccount infoAccount = await Global.GetBankAccountByNameAsync(argus[1]);
                            if (infoAccount != null)
                            {
                                int index = Global.GetCharacterIndexByName(argus[1]);
                                EmbedBuilder eb = new EmbedBuilder();
                                eb.WithTitle($"Argent de **{Global.Characters[index]}**")
                                    .WithColor(this._rand.Next(256), this._rand.Next(256), this._rand.Next(256))
                                    .AddField("==============================================", infoAccount.ToString().Replace(infoAccount.ToString().Split('\n')[0] + '\n', "Disponible: ") + $"\nPropriétaire: {this.Context.Guild.GetUser(Global.Characters[index].OwnerId).Username}");
